# Contributor License Agreement (CLA)

Thank you for your interest in contributing to ZaloCRM (the "Project"),
maintained by Dương Minh Mẫn (the "Maintainer").

This Contributor License Agreement ("Agreement") sets out the terms under which
You provide Contributions to the Project. By submitting a Contribution (for
example, via a pull request), You agree to the terms below. If You are
contributing on behalf of a company or other entity, You represent that You are
authorized to bind that entity to this Agreement.

> This document is a contributor agreement, not legal advice. For a binding,
> jurisdiction-specific agreement, have it reviewed by a qualified lawyer.

## 1. Definitions

- **"You" / "Your"** — the individual or entity making a Contribution.
- **"Contribution"** — any original work of authorship, including any
  modifications or additions to existing work, that You intentionally submit to
  the Project in any form (code, documentation, configuration, or other
  materials).
- **"Submit"** — any form of communication sent to the Project (e.g. pull
  requests, patches, issues with attached code), excluding communication clearly
  marked "Not a Contribution".

## 2. Grant of Copyright License

You hereby grant to the Maintainer a perpetual, worldwide, non-exclusive,
royalty-free, irrevocable copyright license to reproduce, prepare derivative
works of, publicly display, publicly perform, sublicense, distribute, and
otherwise use Your Contributions and such derivative works.

This license **includes the right to license or relicense** Your Contributions,
in whole or in part, under any license terms chosen by the Maintainer, including
both open-source and proprietary terms.

## 3. Grant of Patent License

You hereby grant to the Maintainer and to recipients of software distributed by
the Project a perpetual, worldwide, non-exclusive, royalty-free, irrevocable
(except as stated below) patent license to make, have made, use, offer to sell,
sell, import, and otherwise transfer Your Contributions, where such license
applies only to those patent claims licensable by You that are necessarily
infringed by Your Contributions alone or by combination of Your Contributions
with the Project.

If any entity institutes patent litigation alleging that the Project or a
Contribution constitutes patent infringement, the patent licenses granted under
this Agreement to that entity terminate.

## 4. Your Representations

You represent that:

1. Each Contribution is **Your original creation**, or You have sufficient
   rights to submit it under the terms of this Agreement.
2. Your Contribution does not, to Your knowledge, violate any third party's
   copyright, patent, trademark, trade secret, or other rights.
3. If Your employer has rights to intellectual property You create, You have
   received permission to make the Contribution on behalf of that employer, or
   the employer has waived such rights.

## 5. Disclaimer

Unless required by applicable law or agreed in writing, You provide Your
Contributions on an **"AS IS" basis, without warranties or conditions** of any
kind, either express or implied, including any warranties of title,
non-infringement, merchantability, or fitness for a particular purpose.

## 6. No Obligation

You understand that the decision to include Your Contribution in any product or
release is entirely at the Maintainer's discretion, and this Agreement does not
obligate the Maintainer to use or incorporate Your Contribution.

## 7. How to Accept

You accept this Agreement by either:

- The CLA assistant check on your pull request (if configured); or
- Adding a sign-off line to each commit:
  `Signed-off-by: Your Name <your@email>`; or
- Stating in your pull request: *"I have read and agree to the CLA."*

---

By contributing to this Project, You acknowledge that You have read and agree to
this Agreement.
